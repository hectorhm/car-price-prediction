...
ggplot(countyComplete, aes(x = hs_grad, y = poverty)) + 
  ...
  geom_abline(slope = -0.82, intercept = 85, color  = "red") 
```
...
nossas_estimativas = countyComplete %>% 
  ...
  mutate(
    segundo_modelo = modelo(hs_grad, -0.82, 85), 
...
```
```{r}
fitP <- lm(poverty ~ ., data=countyComplete)
summary(fitP)
```

```{r}
fitP <- lm(poverty ~ hs_grad, data=countyComplete)
summary(fitP)
plot(fitP)
```
